...
public class AbilityRegistry {
    ...
    public AbilityRegistry() {
        this.templates = List.of(
                ...
                template("exploration.trail_sense", "Trail Sense", AbilityCategory.TRAVERSAL_MOBILITY, AbilityFamily.MOBILITY, AbilityTrigger.ON_CHUNK_ENTER, AbilityMechanic.TRAIL_SENSE,
                        "Entering new or rarely visited chunks reveals bounded unexplored direction hints", "extends frontier hint horizon", "drift can bias stale frontier echoes", "awakening stabilizes uncharted bearings", "fusion shares local frontier vectors", "memory logs exploration chain opportunities",
                ...
                template("exploration.cartographers_echo", "Cartographer's Echo", AbilityCategory.TRAVERSAL_MOBILITY, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_STRUCTURE_DISCOVERY, AbilityMechanic.CARTOGRAPHERS_ECHO,
                        "Structure discoveries surface bounded follow-on structure discovery vectors", "chains nearby discovery breadcrumbs", "drift can add low-confidence lead-ins", "awakening improves follow-on confidence", "fusion links neighboring discovery pressure", "memory stores structure chaining corridors",
                ...
                template("gathering.forager_memory", "Forager Memory", AbilityCategory.RESOURCE_FARMING_LOGISTICS, AbilityFamily.SURVIVAL, AbilityTrigger.ON_RESOURCE_HARVEST_STREAK, AbilityMechanic.FORAGER_MEMORY,
                        "Harvest streaks reveal bounded nearby matching-resource extension opportunities", "extends local streak memory", "drift can lock onto low-value extensions", "awakening improves streak fidelity", "fusion shares active streak motifs", "memory tracks harvest-chain density",
                ...
                template("ritual.pattern_resonance", "Pattern Resonance", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CHAOS, AbilityTrigger.ON_REPEATED_BLOCK_PATTERN, AbilityMechanic.PATTERN_RESONANCE,
                        "Repeated player block patterns trigger short-lived local ritual resonance amplification", "extends resonance windows", "drift can resonate ambiguous motifs", "awakening isolates repeatable ritual signatures", "fusion synchronizes nearby pattern resonance", "memory records high-signal ritual cadence",
                ...
                template("social.witness_imprint", "Witness Imprint", AbilityCategory.SOCIAL_SUPPORT_COORDINATION, AbilityFamily.PRECISION, AbilityTrigger.ON_PLAYER_WITNESS, AbilityMechanic.WITNESS_IMPRINT,
                        "Player-observed events imprint witness-aware narrative memory into lineage state", "expands witness memory context", "drift can record noisy observers", "awakening prioritizes reliable witness contexts", "fusion links shared witness records", "memory accumulates witness interactions",
                ...
                template("evolution.entropy_pulse", "Entropy Pulse", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CHAOS, AbilityTrigger.ON_MEMORY_EVENT, AbilityMechanic.UNSTABLE_DETONATION,
                        "Branch divergence spikes mutation intensity for short exploratory windows", "divergence-sensitive mutation cadence", "drift amplifies stochastic branches", "awakening bounds pulse instability", "fusion shares entropy probes", "memory marks divergence shocks",
                ...
                template("evolution.resource_parasitism", "Resource Parasitism", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CHAOS, AbilityTrigger.ON_MEMORY_EVENT, AbilityMechanic.RESOURCE_ECOLOGY_SCAN,
                        "Siphons opportunity from dominant concentrations to increase anti-meta pressure", "improves anti-dominance timing", "drift risks overextension", "awakening stabilizes siphon windows", "fusion triangulates dominant pressure", "memory tracks dominant collapse windows",
                ...
                template("evolution.ritual_amplifier", "Ritual Amplifier", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_RITUAL_COMPLETION, AbilityMechanic.RITUAL_STABILIZATION,
                        "Repeated ritual behaviors deepen utility and coherence with bounded drift", "longer ritual coherence runs", "drift lowers adaptation elasticity", "awakening stabilizes ritual loops", "fusion synchronizes ritual cohorts", "memory preserves ritual chains",
                ...
                template("evolution.lineage_fortification", "Lineage Fortification", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.SURVIVAL, AbilityTrigger.ON_LOW_HEALTH, AbilityMechanic.GUARDIAN_PULSE,
                        "Collapse-prone branches gain temporary resilience at high upkeep cost", "better collapse grace timing", "drift inflates maintenance burden", "awakening hardens vulnerable branches", "fusion shares fortification cues", "memory records survival recoveries",
                ...
                template("evolution.niche_architect", "Niche Architect", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_REPEATED_BLOCK_PATTERN, AbilityMechanic.ECOLOGICAL_PATHING,
                        "Sustained niche occupation reinforces utility density and niche identity", "stronger niche reinforcement slope", "drift reduces cross-niche mobility", "awakening tunes niche persistence", "fusion shares engineered niche signals", "memory captures niche construction history",
                ...
                template("mobility.rift_stride", "Rift Stride", AbilityCategory.TRAVERSAL_MOBILITY, AbilityFamily.MOBILITY, AbilityTrigger.ON_ELEVATION_CHANGE, AbilityMechanic.MOVEMENT_ECHO,
                        "Vertical transitions cache a bounded rebound line for quick terrain correction", "extends rebound safety window", "drift can overshoot landing bands", "awakening stabilizes cliffside routing", "fusion links squad repositioning lanes", "memory stores hard-earned ascent lines",
                ...
                template("mobility.skyline_fold", "Skyline Fold", AbilityCategory.TRAVERSAL_MOBILITY, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_CHUNK_ENTER, AbilityMechanic.CARTOGRAPHIC_ECHO,
                        "Open-sky chunk entry compresses broad routes into safe traversal bands", "widens corridor certainty", "drift can favor scenic detours", "awakening filters dead-end skylines", "fusion overlays convoy route folds", "memory ranks the safest long-haul routes",
                ...
                template("sensing.contraband_tell", "Contraband Tell", AbilityCategory.SENSING_INFORMATION, AbilityFamily.PRECISION, AbilityTrigger.ON_PLAYER_TRADE, AbilityMechanic.MARK,
                        "Trades leave short-lived tells that hint whether goods came from hidden stockpiles", "improves suspicious-route confidence", "drift can overflag common wares", "awakening filters normal market churn", "fusion shares black-market heuristics", "memory tracks recurring suspicious signatures",
                ...
                template("sensing.battlefield_read", "Battlefield Read", AbilityCategory.COMBAT_TACTICAL_CONTROL, AbilityFamily.PRECISION, AbilityTrigger.ON_PLAYER_WITNESS, AbilityMechanic.INSIGHT_REVEAL,
                        "Witnessed clashes expose temporary spacing and flank pressure hints", "deeper skirmish geometry reads", "drift can overreact to feints", "awakening isolates genuine pressure lines", "fusion broadcasts threat contours to allies", "memory stores recurring duel patterns",
                ...
                template("survival.hardiness_loop", "Hardiness Loop", AbilityCategory.SURVIVAL_ADAPTATION, AbilityFamily.SURVIVAL, AbilityTrigger.ON_WEATHER_CHANGE, AbilityMechanic.RECOVERY_WINDOW,
                        "Sudden weather swings grant a bounded attrition-recovery loop for harsh travel", "extends adaptation duration", "drift can trigger on mild fronts", "awakening smooths extreme climate pivots", "fusion shares resilience tempo nearby", "memory learns the cost of stubborn routes",
                ...
                template("survival.storm_shelter_ledger", "Storm Shelter Ledger", AbilityCategory.SURVIVAL_ADAPTATION, AbilityFamily.SURVIVAL, AbilityTrigger.ON_STRUCTURE_PROXIMITY, AbilityMechanic.DEFENSIVE_THRESHOLD,
                        "Shelter thresholds remember recent weather exposure and open a staged refuge window before the next front lands", "extends refuge grace depth", "drift can bind to unsafe ruins", "awakening separates shelter from trap geometry", "fusion links fallback shelters into a storm chain", "memory preserves shelters that truly bought time",
                ...
                template("survival.exposure_weave", "Exposure Weave", AbilityCategory.SURVIVAL_ADAPTATION, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_TIME_OF_DAY_TRANSITION, AbilityMechanic.TEMPORAL_SPECIALIZATION,
                        "Repeated day-night exposure in the same biome builds a short adaptation weave that swaps between travel resilience and camp efficiency", "improves weave duration", "drift can invert the preferred phase", "awakening preserves the useful half of the cycle", "fusion shares exposure rhythm across nearby allies", "memory records which climates demanded patience",
                ...
                template("survival.last_light_cache", "Last Light Cache", AbilityCategory.DEFENSE_WARDING, AbilityFamily.SURVIVAL, AbilityTrigger.ON_TIME_OF_DAY_TRANSITION, AbilityMechanic.GUARDIAN_PULSE,
                        "Nightfall tightens safe-radius instincts around recently tended shelters", "widens dusk shelter memory", "drift can bond to weak camps", "awakening sharpens panic-to-shelter transitions", "fusion synchronizes party fallback beacons", "memory preserves proven refuge rings",
                ...
                template("tactical.killzone_lattice", "Killzone Lattice", AbilityCategory.COMBAT_TACTICAL_CONTROL, AbilityFamily.BRUTALITY, AbilityTrigger.ON_STRUCTURE_PROXIMITY, AbilityMechanic.BATTLEFIELD_FIELD,
                        "Fortified spaces sketch short-lived pressure lanes for chokepoint control", "improves choke coverage", "drift can overcommit to bad holds", "awakening sharpens lane overlap", "fusion chains allied pressure zones", "memory ranks successful defense geometries",
                ...
                template("tactical.feint_window", "Feint Window", AbilityCategory.COMBAT_TACTICAL_CONTROL, AbilityFamily.CHAOS, AbilityTrigger.ON_SOCIAL_INTERACT, AbilityMechanic.CHAIN_ESCALATION,
                        "Intentional player interaction can seed a fake opening before conflict starts", "extends bluff viability", "drift can bluff into dead angles", "awakening separates bait from panic", "fusion layers coordinated feints", "memory tracks which opponents bite",
                ...
                template("tactical.tempo_extract", "Tempo Extract", AbilityCategory.COMBAT_TACTICAL_CONTROL, AbilityFamily.PRECISION, AbilityTrigger.ON_CHAIN_COMBAT, AbilityMechanic.BATTLEFIELD_FIELD,
                        "Extended skirmish chains expose a tempo window where one more push or disengage is favored before the fight flips", "deepens tempo-window confidence", "drift can call the swing too early", "awakening separates real momentum from noise", "fusion shares timing windows across allies", "memory keeps the cadence of fights that turned on patience",
                ...
                template("warding.perimeter_hum", "Perimeter Hum", AbilityCategory.DEFENSE_WARDING, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_STRUCTURE_PROXIMITY, AbilityMechanic.DEFENSIVE_THRESHOLD,
                        "Owned thresholds emit a low hum when perimeter rhythm is broken", "expands ward footprint", "drift can answer harmless movement", "awakening filters routine traffic", "fusion links layered base wards", "memory encodes trusted perimeter cadence",
                ...
                template("warding.sanctum_lock", "Sanctum Lock", AbilityCategory.DEFENSE_WARDING, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_RITUAL_COMPLETION, AbilityMechanic.RITUAL_STABILIZATION,
                        "Completed maintenance rites harden workshop and storage stability for a short cycle", "extends sanctum uptime", "drift can lock low-value rooms", "awakening narrows to critical infrastructure", "fusion shares sanctum state across linked bases", "memory prioritizes reliable strongholds",
                ...
                template("warding.fault_survey", "Fault Survey", AbilityCategory.DEFENSE_WARDING, AbilityFamily.PRECISION, AbilityTrigger.ON_BLOCK_INSPECT, AbilityMechanic.INSIGHT_REVEAL,
                        "Inspecting walls, supports, or machine housings reveals stress seams that would fail first under intrusion or neglect", "deepens seam-read fidelity", "drift can obsess over cosmetic scars", "awakening separates true fault lines from old repairs", "fusion links multiple inspections into one fortification plan", "memory stores the weak spots that nearly cost the base",
                ...
                template("warding.anchor_cadence", "Anchor Cadence", AbilityCategory.DEFENSE_WARDING, AbilityFamily.MOBILITY, AbilityTrigger.ON_PLAYER_GROUP_ACTION, AbilityMechanic.COLLECTIVE_RELAY,
                        "Repeated patrol or upkeep loops teach a base its anchor cadence, revealing where relief coverage is late or overstacked", "extends cadence lookback", "drift can canonize sloppy patrols", "awakening filters busywork from real coverage", "fusion synchronizes linked sentry circuits", "memory records which patrol rhythms actually deterred breaches",
                ...
                template("warding.false_threshold", "False Threshold", AbilityCategory.DEFENSE_WARDING, AbilityFamily.CHAOS, AbilityTrigger.ON_STRUCTURE_PROXIMITY, AbilityMechanic.MARK,
                        "Repeated edge-testing by outsiders can seed a decoy safe entry that wastes the intruder's next committed approach", "extends decoy-threshold persistence", "drift can mislabel friendly traffic as probing", "awakening reserves false welcomes for hostile patterns", "fusion braids multiple decoy entries around a fort", "memory remembers which fake openings actually diverted raiders",
                ...
                template("logistics.convoy_instinct", "Convoy Instinct", AbilityCategory.RESOURCE_FARMING_LOGISTICS, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_PLAYER_GROUP_ACTION, AbilityMechanic.COLLECTIVE_RELAY,
                        "Hauling as a group reveals the least-friction delivery leg for the convoy", "extends convoy relay distance", "drift can route through risky shortcuts", "awakening filters ambush-prone links", "fusion synchronizes multi-run supply timing", "memory records dependable haul circuits",
                ...
                template("logistics.stockpile_tide", "Stockpile Tide", AbilityCategory.RESOURCE_FARMING_LOGISTICS, AbilityFamily.SURVIVAL, AbilityTrigger.ON_ITEM_PICKUP, AbilityMechanic.HARVEST_RELAY,
                        "Pickup streaks expose whether nearby stores are draining or saturating", "improves refill timing", "drift can overread temporary spikes", "awakening distinguishes throughput from clutter", "fusion shares stock warnings to allies", "memory tracks stable refill thresholds",
                ...
                template("logistics.spoilage_audit", "Spoilage Audit", AbilityCategory.RESOURCE_FARMING_LOGISTICS, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_BLOCK_INSPECT, AbilityMechanic.MEMORY_ECHO,
                        "Inspecting farms, chests, or processors reveals where delay or overstock is silently turning output into waste", "improves waste-trail fidelity", "drift can moralize harmless stockpiles", "awakening distinguishes reserve stock from real decay", "fusion links audits across a workshop chain", "memory keeps the loss patterns worth fixing first",
                ...
                template("engineering.redstone_sympathy", "Redstone Sympathy", AbilityCategory.CRAFTING_ENGINEERING_AUTOMATION, AbilityFamily.PRECISION, AbilityTrigger.ON_BLOCK_INSPECT, AbilityMechanic.STRUCTURE_ATTUNEMENT,
                        "Workshop inspection highlights likely machine intent and missing links", "improves machine-state confidence", "drift can admire decorative noise", "awakening separates logic from ornament", "fusion shares workshop diagnostics", "memory catalogs stable machine motifs",
                ...
                template("engineering.pattern_forge", "Pattern Forge", AbilityCategory.CRAFTING_ENGINEERING_AUTOMATION, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_REPEATED_BLOCK_PATTERN, AbilityMechanic.PATTERN_RESONANCE,
                        "Repeated construction motifs condense into a bounded fabrication cadence bonus", "extends build cadence chains", "drift can reinforce inefficient loops", "awakening trims wasteful pattern echoes", "fusion shares template cadence with nearby builders", "memory favors repeatable workshop routines",
                ...
                template("engineering.machine_rhythm", "Machine Rhythm", AbilityCategory.CRAFTING_ENGINEERING_AUTOMATION, AbilityFamily.CHAOS, AbilityTrigger.ON_MEMORY_EVENT, AbilityMechanic.TEMPORAL_SPECIALIZATION,
                        "Remembered workshop failures and successes settle into a machine rhythm that hints when to pulse, pause, or batch a finicky process", "deepens rhythm confidence", "drift can canonize lucky timing", "awakening separates true cycle rhythm from superstition", "fusion aligns shared workshop timing across artifacts", "memory keeps the process beats worth repeating",
                ...
                template("support.rally_ledger", "Rally Ledger", AbilityCategory.SOCIAL_SUPPORT_COORDINATION, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_PLAYER_GROUP_ACTION, AbilityMechanic.SOCIAL_ATTUNEMENT,
                        "Coordinated actions annotate who is anchoring the group's tempo and who is lagging behind", "improves group tempo clarity", "drift can overvalue loud participants", "awakening favors reliable anchors", "fusion aligns multiple subgroups", "memory stores effective team rhythms",
                ...
                template("support.mercy_link", "Mercy Link", AbilityCategory.SOCIAL_SUPPORT_COORDINATION, AbilityFamily.SURVIVAL, AbilityTrigger.ON_PLAYER_WITNESS, AbilityMechanic.RECOVERY_WINDOW,
                        "Watching allies under pressure opens a short shared recovery timing window", "extends clutch-support timing", "drift can trigger for low-stakes scrapes", "awakening reserves windows for real danger", "fusion spreads aid cadence through the squad", "memory remembers who reciprocates",
                ...
                template("support.role_call", "Role Call", AbilityCategory.SOCIAL_SUPPORT_COORDINATION, AbilityFamily.PRECISION, AbilityTrigger.ON_SOCIAL_INTERACT, AbilityMechanic.SOCIAL_ATTUNEMENT,
                        "Brief social check-ins reveal who is carrying the wrong role load before a plan starts", "deepens role-read accuracy", "drift can overtrust confident talkers", "awakening favors proven responsibility over volume", "fusion aligns role swaps across subgroups", "memory stores which teams redistributed labor in time",
                ...
                template("support.cover_exchange", "Cover Exchange", AbilityCategory.SOCIAL_SUPPORT_COORDINATION, AbilityFamily.CHAOS, AbilityTrigger.ON_PLAYER_TRADE, AbilityMechanic.TRADE_SCENT,
                        "Seemingly routine item handoffs can quietly exchange responsibilities, safe words, or fallback plans without exposing the real coordinator", "extends cover-story durability", "drift can swap roles at the wrong moment", "awakening reserves the exchange for meaningful pressure", "fusion lets allied crews rotate responsibility without panic", "memory keeps the handoff patterns that protected the team",
                ...
                template("ritual.oath_circuit", "Oath Circuit", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CHAOS, AbilityTrigger.ON_PLAYER_GROUP_ACTION, AbilityMechanic.ALTAR_SIGNAL_BOOST,
                        "Synchronized group gestures turn social commitment into bounded ritual throughput", "extends oath-chain length", "drift can bind empty gestures", "awakening filters unserious participants", "fusion braids multiple oath circles", "memory preserves successful communal rites",
                ...
                template("ritual.moon_debt", "Moon Debt", AbilityCategory.RITUAL_STRANGE_UTILITY, AbilityFamily.CHAOS, AbilityTrigger.ON_TIME_OF_DAY_TRANSITION, AbilityMechanic.MEMORY_ECHO,
                        "Night transitions surface what the land still 'owes' from unfinished rites or expeditions", "deepens unpaid-ritual read", "drift can blame the wrong frontier", "awakening narrows debts to actionable omens", "fusion shares omen burdens among companions", "memory records places that call players back",
                ...
                template("stealth.hushwire", "Hushwire", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.CHAOS, AbilityTrigger.ON_STRUCTURE_PROXIMITY, AbilityMechanic.MOVEMENT_ECHO,
                        "Approaching defended spaces lays a silent ingress line that suppresses obvious movement tells", "extends hush line reach", "drift can mute allies at bad times", "awakening narrows suppression to infiltration windows", "fusion chains multiple ingress routes", "memory stores the quietest breaches",
                ...
                template("stealth.paper_trail", "Paper Trail", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.PRECISION, AbilityTrigger.ON_PLAYER_TRADE, AbilityMechanic.TRADE_SCENT,
                        "Market interactions can hide or misdirect the scent of recent movement and storage", "improves false-ledger durability", "drift can hide the wrong route", "awakening sharpens smuggler-safe traces", "fusion coordinates distributed cover stories", "memory tracks believable trade alibis",
                ...
                template("stealth.shadow_proxy", "Shadow Proxy", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.CHAOS, AbilityTrigger.ON_PLAYER_WITNESS, AbilityMechanic.MARK,
                        "Being seen lets the artifact plant a false last-known position that only matures if line of sight is broken and the route forks", "extends false-position persistence", "drift can seed obvious decoys", "awakening delays the proxy until a believable split appears", "fusion braids multiple false exits", "memory catalogs which pursuers trust the first clue",
                ...
                template("stealth.dead_drop_lattice", "Dead Drop Lattice", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.PRECISION, AbilityTrigger.ON_PLAYER_TRADE, AbilityMechanic.NAVIGATION_ANCHOR,
                        "A suspicious trade can seed a hidden handoff route, but only if the carrier threads multiple low-traffic waypoints before doubling back", "adds one more viable handoff hop", "drift can overvalue exposed shortcuts", "awakening filters routes already watched by rivals", "fusion lets crews split and rejoin the lattice", "memory stores which drop chains stayed uncompromised",
                ...
                template("stealth.echo_shunt", "Echo Shunt", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.MOBILITY, AbilityTrigger.ON_MOVEMENT, AbilityMechanic.MOVEMENT_ECHO,
                        "Alternating sprint and crouch cadence can split movement noise into one advancing trail and one stalling phantom branch", "extends phantom branch lifetime", "drift can split the real escape path", "awakening sharpens cadence windows for clean shunts", "fusion layers multiple phantom routes across a breach", "memory records when noise was worth spending",
                ...
                template("stealth.ghost_shift", "Ghost Shift", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.MOBILITY, AbilityTrigger.ON_REPOSITION, AbilityMechanic.MARK,
                        "Hard lateral repositioning can overwrite a tracker's last assumed angle, but only if the move crosses a believable obstacle edge", "extends overwrite persistence", "drift can spend the shift on obvious jukes", "awakening waits for convincing obstacle breaks", "fusion layers multiple cross-angles into one escape story", "memory records which sidesteps actually shed pursuit",
                ...
                template("stealth.trace_fold", "Trace Fold", AbilityCategory.STEALTH_TRICKERY_DISRUPTION, AbilityFamily.CONSISTENCY, AbilityTrigger.ON_BLOCK_INSPECT, AbilityMechanic.MEMORY_ECHO,
                        "Reading a surface before passing through it can fold your freshest trace into older disturbance, making pursuit forensics lose the correct timestamp", "deepens trace-fold confidence", "drift can bury the route in unrelated clutter", "awakening separates usable cover from noisy history", "fusion lets crews stagger their traces into one older story", "memory records which materials hid passage best",
    ...
    private AbilityTemplate template(String id, String name, AbilityCategory category, AbilityFamily family, AbilityTrigger trigger, AbilityMechanic mechanic,
        ...
        return new AbilityTemplate(id, name, category, family, trigger, mechanic, effectPattern, evolutionVariant, driftVariant, awakeningVariant, fusionVariant,
