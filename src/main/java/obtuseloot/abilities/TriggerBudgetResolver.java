...
public final class TriggerBudgetResolver {
    ...
    private TriggerBudgetProfile defaultsFor(AbilityDefinition ability, AbilityTrigger trigger) {
        return switch (trigger) {
            case ON_MOVEMENT, ON_WORLD_SCAN, ON_CHUNK_ENTER, ON_BIOME_CHANGE, ON_TIME_OF_DAY_TRANSITION, ON_WEATHER_CHANGE, ON_ELEVATION_CHANGE -> new TriggerBudgetProfile(1.3D, 0.8D, 10.0D, 3.2D, 3, 1200L, 35, TriggerBudgetPolicy.PASSIVE_LOW_PRIORITY, false, 250.0D);
            case ON_STRUCTURE_SENSE, ON_STRUCTURE_DISCOVERY, ON_STRUCTURE_PROXIMITY -> new TriggerBudgetProfile(2.2D, 1.1D, 9.0D, 2.4D, 2, 2000L, 30, TriggerBudgetPolicy.STRICT, false, 450.0D);
            case ON_MEMORY_EVENT, ON_WITNESS_EVENT, ON_PLAYER_WITNESS -> new TriggerBudgetProfile(1.1D, 0.5D, 11.0D, 3.8D, 3, 1300L, 45, TriggerBudgetPolicy.SOFT, false, 200.0D);
            case ON_RITUAL_INTERACT, ON_ENTITY_INSPECT, ON_BLOCK_INSPECT, ON_BLOCK_HARVEST, ON_SOCIAL_INTERACT,
                    ON_RESOURCE_HARVEST_STREAK, ON_ITEM_PICKUP, ON_REPEATED_BLOCK_PATTERN, ON_RITUAL_COMPLETION,
                    ON_PLAYER_GROUP_ACTION, ON_PLAYER_TRADE -> new TriggerBudgetProfile(0.8D, 0.2D, 14.0D, 6.5D, 6, 700L, 85, TriggerBudgetPolicy.ACTIVE_INTENTIONAL, true, 60.0D);
            default -> {
    ...
    private boolean isIntentional(AbilityEventContext context) {
        ...
        if (trigger == AbilityTrigger.ON_RITUAL_INTERACT || trigger == AbilityTrigger.ON_ENTITY_INSPECT
                ...
                || trigger == AbilityTrigger.ON_SOCIAL_INTERACT || trigger == AbilityTrigger.ON_RESOURCE_HARVEST_STREAK
                || trigger == AbilityTrigger.ON_ITEM_PICKUP || trigger == AbilityTrigger.ON_REPEATED_BLOCK_PATTERN
                || trigger == AbilityTrigger.ON_RITUAL_COMPLETION || trigger == AbilityTrigger.ON_PLAYER_GROUP_ACTION
                || trigger == AbilityTrigger.ON_PLAYER_TRADE) {
            return true;
        }
