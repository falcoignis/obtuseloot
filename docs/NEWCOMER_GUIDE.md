...
- `src/main/java/obtuseloot/ObtuseLoot.java`: plugin entrypoint, config/data loading, artifact generation, event hooks, and command handling.
- `src/main/java/obtuseloot/engine/SoulEngine.java`: runtime soul effect engine (particles, ability behaviors, scheduled tasks, and combat/movement hooks).
- `src/main/java/obtuseloot/data/`: immutable records (`SoulData`, `PlayerSoulState`) shared between loader and runtime engine.
- `src/main/java/obtuseloot/lore/` and `.../names/`: curated default text pools used when first creating data files.
